...
cur = db.cursor()
cur.execute("use attendance")

def mark_attendance(sub,name,date): 
    cur.execute(f"insert into {sub} values('{name}','{date}')")
...
while True:
    ...
    for (top,right, bottom, left), face_encoding in zip(face_location, face_encodings):
        ...
        if True in matches:
            ...
            mark_attendance("dbms",name,datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
